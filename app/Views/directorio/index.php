  ...
  <section class="hero" style="padding: 3rem 0;">
    <div class="container">
      <h1 class="hero-title" style="text-align: center; margin-bottom: 1rem;">
        Directorio Completo de <span id="total-count">...</span> SOFOMes en M&eacute;xico
      </h1>
      ...
      <div class="filters">
        ...
        <div class="filter-group">
          ...
          <select id="filter-estatus">
            ...
            <option value="En operación" selected>En operaci&oacute;n</option>
          </select>
        </div>
      </div>
        ...
        <table id="sofomes-table">
          <thead>
            <tr>
              ...
              <th class="sortable" data-column="denominacion">Denominaci&oacute;n Social</th>
...
<script>
  ...
  async function init() {
    try {
      const response = await fetch('<?= site_url('api/condusef-proxy') ?>');
      const data = await response.json();

      if (data.emergency_fallback) {
        console.warn('CONDUSEF no disponible. Usando cache de emergencia.');
